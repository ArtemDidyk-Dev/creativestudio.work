    ...
    <section class="contacts">
        <div class="container">
            ...
            <div class="contacts__wrapper">
                <div class="contacts__left">
                    ...
                    <div class="contacts__info-subtitle">
                        Get in touch with the right person or team by choosing what kind of assistance you are looking for
                    </div>
                    @if (!empty(setting('address', true)))
                        <a href="{{ setting('map') }}" class="contacts__info-line">
                            <img src="{{ asset('build/website/images/icons/contacts-location.svg') }}" alt="" class="contacts__info-line-img">
                    ...
                    @if (!empty(setting('email')))
                        <a href="mailto:{{ setting('email') }}" class="contacts__info-line">
                            <img src="{{ asset('build/website/images/icons/contacts-email.svg') }}" alt="" class="contacts__info-line-img">
                            <div class="contacts__info-line-text contacts">
                                {{ setting('email') }}
                            </div>
                        </a>
                ...
                <div class="contacts__right">
                    <x-inc.standart.image-content width="620" hight="408" src="{{ asset('build/website/images/other/contacts.png') }}"
                        alt="contacts" />
                </div>
            </div>
            ...
            <form action="{{ route('frontend.home.contactSendAjax') }}" method="POST" class="contacts-form">
                @csrf
                <h2>
                    {{ language('Ready to help you') }}
                </h2>
                <div class="contacts-form__top">
                    <div class="contacts-form__top-element contacts-form-input-sm">
                        <x-inc.inputs.input label="Name">
                            <x-inc.inputs.text name="name" placeholder="{{ language('frontend.contact.your_name') }}" />
                        </x-inc.inputs.input>
                    </div>
                    <div class="contacts-form__top-element contacts-form-input-sm">
                        <x-inc.inputs.input label="Email">
                            <x-inc.inputs.text name="email" placeholder="{{ language('frontend.contact.your_email') }}" />
                        </x-inc.inputs.input>
                    </div>
                    <div class="contacts-form__top-element contacts-form-input">
                        <x-inc.inputs.input label="Subject">
                            <x-inc.inputs.text placeholder="{{ language('frontend.contact.your_subject') }}" name="subject" />
                        </x-inc.inputs.input>
                    </div>
                </div>
                <div class="contacts-form__bottom">
                    ...
                    <x-inc.btns.filter color="black" title="Send" />
                </div>
